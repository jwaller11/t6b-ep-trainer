export const briefs = [
  {
    ...
    "steps": [
      ...
      {
        "type": "action",
        "text": "(1) In the event of a radio or ICS failure, we will troubleshoot in an attempt to reestablish comms or ICS (i.e. check communications leads all the way to the O2 mask, the comm panel, and UFCP for appropriate frequencies and switchology) (2) If we have a radio failure, we will attempt communication on another radio, using the standby VHF if necessary. If we have a total loss of communications in the local area, we will comply with the local letter of agreement for IFR and VFR aircraft. If we are outside of the local area, we will comply with the FIH (3) If we have an ICS failure, we can remove our masks momentarily and shout or use frequencies 123.45 or 246.8 to communicate as necessary. If ICS cannot be restored, the instructional portion of the flight will be terminated, and we will land as soon as practical"
      },
      {
        "type": "condition",
      ...
      {
        "type": "action",
        "text": "Instrument conditions are to be avoided at all times while operating VFR. If actual instrument flight is encountered: (a) Maintain aircraft control (b) Establish an instrument scan and check altitude (c) If altitude below Maximum Elevation Figure (MEF) + 1000 feet or Off Route Obstruction Clearance Altitude (OROCA), execute immediate climb to safe altitude turning away from known obstacles. Do not attempt to regain VMC below the calculated or published safe altitude. (d) If altitude above MEF + 1000 feet or OROCA, with smooth coordinated control inputs get out of IMC the way it was entered if climbing→descend (etc.). (e) If unable to regain VMC, the IP will contact the nearest ATC facility and coordinate an IFR clearance. If swift contact cannot be made, Squawk 7700, declare an emergency, and use guard as appropriate"
      },
      {
        "type": "condition",
      ...
      {
        "type": "action",
        "text": "If we are first on scene to an aircraft mishap, we will assume on-scene commander duties. The flying pilot will establish the aircraft at a safe altitude and distance to maintain visual contact and the non-flying pilot will initiate the on-scene commander checklist. We will set a BINGO to the nearest suitable field and remain on-scene until we: 1) Reach our BINGO fuel; 2) Have an emergency of our own; 3) Are relieved by a more appropriate platform; 4) The rescue is complete. If we are not first on scene, we may offer assistance but will remain clear unless called upon"
      },
      {
        "type": "condition",
      ...
      {
        "type": "action",
        "text": "(1) “If we are in Out-of-Controlled Flight, we will execute the INADVERTENT DEPARTURE FROM CONTROLLED FLIGHT Emergency Procedure; PCL IDLE, controls neutral, altitude check, recover from unusual altitude (2) “OCF can be identified if the aircraft does not respond immediately and in a normal sense to application of flight controls. Airspeed in a steady-state spin will either be stable or it will oscillate above and below a constant airspeed, while the turn needle will be relied upon to indicate direction of rotation"
      },
      {
        "type": "condition",
      ...
      {
        "type": "action",
        "text": "(1) “Ejection is never simulated (2) Immediate. “If impact is imminent with insufficient time for the normal cadence, we will both immediately pull the ejection handle (3) Time Critical (a) “To facilitate proper body position, the call for ejection will normally be a foursecond cadence: ‘EJECT, EJECT, EJECT,’ pull (both) the ejection handle. In the event of an ICS failure, execute three raps on the canopy, pull (both). (b) “Ensure proper body position: back and musters against the seat, head on the head rest, chin up 10 degrees, feet on the rudder pedals, and elbows in tight toward the body. The minimum recommended altitude for uncontrolled ejection is 6,000 feet AGL, and 2,000 feet AGL for a controlled ejection (4) Deliberate. “Time permitting, we will execute as many of the CONTROLLED EJECTION checklist steps as possible. The Controlled Ejection Area is defined as the CRP 170 radial at 20 DME (Chapman Ranch) heading 210"
      }
    ]
  },
  {
